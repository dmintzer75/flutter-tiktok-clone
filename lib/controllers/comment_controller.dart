...
class CommentController extends GetxController {
  ...
  getComment() async {
    _comments.bindStream(
      firebaseFirestore.collection('videos').doc(_postId).collection('comments').snapshots().map(
        (QuerySnapshot query) {
  ...
  likeComment(String id) async {
    ...
    DocumentSnapshot doc =
        await firebaseFirestore.collection('videos').doc(_postId).collection('comments').doc(id).get();

    if ((doc.data()! as dynamic)['likes'].contains(id)) {
